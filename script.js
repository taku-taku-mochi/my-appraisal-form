    ...
    function addItemBlock(isOpen = false) {
        ...
        itemBlock.innerHTML = `
            <div class="accordion-header hover:bg-gray-200">
                <h3 class="text-lg font-semibold text-gray-800"><span class="item-number"></span><span class="item-summary text-gray-500 text-sm ml-4"></span></h3>
            ...
            <div class="accordion-content space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2"><textarea name="itemNotes" rows="2" class="block w-full px-4 py-2 border border-gray-300 rounded-md form-input" placeholder="ご要望・備考"></textarea></div>
                </div>
                <div class="mt-4">
                    <label class="block text-sm font-medium text-gray-600">オプション</label>
                    <div class="mt-2 grid grid-cols-2 gap-3">
                        ${Object.keys(OPTION_PRICES).map(option => `<label class="flex items-center"><input type="checkbox" name="itemOptions" value="${option}" class="rounded-md text-blue-600 focus:ring-blue-500 w-5 h-5"><span class="ml-2 text-sm text-gray-700">${option}</span></label>`).join('')}
                    </div>
                </div>
